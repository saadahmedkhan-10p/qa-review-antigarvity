                ...
                <div className="space-y-12">
                    <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700/50 transition-colors">
                        <div className="px-8 py-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/30">
                            <h2 className="text-2xl font-black text-gray-800 dark:text-white tracking-tight">Assigned Projects ({filteredProjects.length})</h2>
                        </div>
                        <div className="overflow-x-auto">
                            {filteredProjects.length === 0 ? (
                                <p className="p-12 text-gray-400 dark:text-gray-500 text-center text-lg font-medium italic">No assigned projects found.</p>
                            ) : (
                                <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                                    <thead className="bg-gray-50/30 dark:bg-gray-800/20">
                                        <tr>
                                            <th className="px-8 py-4 text-left text-xs font-black text-gray-700 dark:text-gray-500 uppercase tracking-widest">Project / Type / Form</th>
                                    ...
                                    <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                                        {filteredProjects.map((project: any) => {
                                            ...
                                            const isSecondary = project.secondaryReviewerId === user.id;

                                            // If there's no review record yet
                                            if (!review) {
                                                return (
                                                    <tr key={project.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                                                        ...
                                                        <td className="px-8 py-6 text-gray-400 dark:text-gray-500 text-xs italic">
                                            ...
                                            // If there is an active/completed review record
                                            ...
                                            return (
                                                <tr key={project.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                                                    <td className="px-8 py-6">
                                                        ...
                                                        <div className="text-xs text-indigo-600 dark:text-indigo-400 font-semibold mb-1">{project.type}</div>
                                                        <div className="text-xs text-gray-500 dark:text-gray-400 font-medium italic">{review.form.title}</div>
                                                    </td>
                                                    <td className="px-8 py-6">
                                                        <div className="text-sm font-medium text-gray-800 dark:text-gray-300">L: {project.lead?.name || 'N/A'}</div>
                                                        <div className="text-sm text-gray-700 dark:text-gray-500">QC: {project.contactPerson?.name || 'N/A'}</div>
                                                    ...
                                                    <td className="px-8 py-6">
                                                        <div className="flex flex-col gap-1 items-start">
                                                            <span className={`px-3 py-1 text-[10px] rounded-full font-black uppercase tracking-wider ${
                                                                review.status === 'SCHEDULED' ? 'bg-blue-200 text-blue-900 dark:bg-blue-900/40 dark:text-blue-300' :
                                                                review.status === 'PENDING' ? 'bg-amber-200 text-amber-900 dark:bg-amber-900/40 dark:text-amber-300' :
                                                            ...
                                                            {review.submittedDate && (
                                                                <div className="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold uppercase tracking-tight">
                                                                    SUB: {format(new Date(review.submittedDate), 'MMM d, yyyy')}
                                                                </div>
                                                            )}
                                                            {isTerminal && review.status === 'SUBMITTED' && (
                                                                <span className={`px-2 py-0.5 mt-1 text-[9px] font-black uppercase rounded border ${
                                                                    review.healthStatus === 'On Track' ? 'bg-green-100 text-green-900 border-green-300 dark:bg-green-900/20 dark:text-green-400' :
                                                            ...
                                                            {isTerminal && review.status !== 'SUBMITTED' && (
                                                                <div className="text-xs font-medium text-gray-500 dark:text-gray-400 italic max-w-xs break-words mt-1">
                                                                    Reason: {
                                                    ...
                                                    <td className="px-8 py-6">
                                                        <div className="flex flex-col gap-3 max-w-[240px]">
                                                            ...
                                                            {isTerminal && (
                                                                <Link
                                                                    href={`/reviews/${review.id}/conduct`}
                                                                    className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 font-black text-sm tracking-tight flex items-center gap-1 group justify-center"
                                                                >
                                                                    VIEW DETAILS
